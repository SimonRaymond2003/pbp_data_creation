...
get_team_injury_counts <- function(team, game_date) {
  ...
  bind_cols(year_injuries, week_injuries)
}
...
```{r}
# Extract all unique injury types
injuries_data <- load_injuries(2016:2023) %>%
  select(practice_primary_injury) %>%
  distinct() %>%
  filter(!is.na(practice_primary_injury)) %>%
  arrange(practice_primary_injury)

# Print the unique injury types
print("Unique injury types:")
print(injuries_data)
```
