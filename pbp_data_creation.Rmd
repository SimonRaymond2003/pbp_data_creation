...
add_player_info <- function(df, players_data, player_col, prefix) {
  ...
  # Inside process_year function, after loading play by play data but before processing
# Determine max regular season week based on year
max_week <- if(year >= 2021) 18 else 17

# Filter both third and fourth downs for regular season only
...
# Get the specialists based on actual game play
get_specialists <- function(year) {
  # Load play by play data for current and previous year
  pbp <- bind_rows(
    load_pbp(year),
    load_pbp(year - 1)
  ) %>%
  arrange(game_date, desc(play_id))  # Most recent plays first
  
  # Get last kicker to attempt FG/XP for each team/week
  kickers <- pbp %>%
  ...
  # Get last punter to punt for each team/week  
  punters <- pbp %>%
    filter(punt_attempt == 1) %>%
    select(team = posteam, week, season, punter_player_id, punter_player_name) %>%
    distinct(team, week, season, .keep_all = TRUE)
    
  # For each play, get the most recent specialist prior to that game
  list(kickers = kickers, punters = punters)
}

# Then modify your join to use this historical perspective:
...
# identifying row duplication

```{r}
cfd <- read.csv("cfd.csv")
cfd <- cfd[c(10002:10004), ]
# selects these rows that are duplicates
```
...
get just those cols [1] "k_name"    "k_gsis_id" "p_name"    "p_gsis_id"


```{r}
cfd4 <- cfd %>% select(k_name, k_gsis_id, p_name, p_gsis_id)
```
